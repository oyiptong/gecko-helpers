...
XPCSHELL_COMMAND = ['./mach', 'xpcshell-test']
MOCHITEST_COMMAND = ['./mach', 'mochitest']
GIT_CURRENT_BRANCH = ['git', 'rev-parse', '--abbrev-ref', 'HEAD']
MOZCONFIG_DEBUG_OPTS = ['--disable-optimize', '--enable-debug']

if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Test Firefox in a sane way')
    parser.add_argument('-x', '--xpc-shell', dest='xpcshell_test', action='store_true', default=False, help='Run XPCShell Tests')
    parser.add_argument('-m', '--mochitest', dest='mochitest', action='store_true', default=False, help='Run Mochitests')
    ...
    if args.xpcshell_test and args.mochitest:
        print "Can't run both XPC and Mochitests. Please choose one\n\n"
        parser.print_help()
        sys.exit(1)
    elif not (args.xpcshell_test or args.mochitest):
        print "Please choose one of xpcshell-test or mochitest\n\n"
        parser.print_help()
        sys.exit(1)
    elif args.xpcshell_test:
        test_cmd = XPCSHELL_COMMAND
    elif args.mochitest:
        test_cmd = MOCHITEST_COMMAND

    test_cmd.extend(args.infiles)
...
{2}""".format(mozconfig_filename, mozconfig_file_content, ' '.join(test_cmd))
        if args.dry_run:
            sys.exit(0)
